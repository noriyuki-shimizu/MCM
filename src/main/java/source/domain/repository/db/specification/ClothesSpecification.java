...
public class ClothesSpecification {
    ...
    public static Specification<Clothes> brandIdContains(final Long brandId) {
        return brandId == null ? null : (root, query, cb) -> cb.equal(root.join("brand", JoinType.INNER).get("id"), brandId);
    }
    ...
    public static Specification<Clothes> hasGenres(final Long genreId) {
        return (root, query, cb) -> {
            ...
            genreSubquery.where(cb.equal(genre.get("id"), genreId), cb.isMember(root, genreClothes));
            return cb.exists(genreSubquery);
        };
    }

    public static Specification<Clothes> shopIdEqual(final Long shopId) {
        return shopId == null ? null : (root, query, cb) -> cb.equal(root.join("shop", JoinType.INNER).get("id"), shopId);
    }
    ...
    public static Specification<Clothes> buyDateEqual(final Date buyDate) {
        return buyDate == null ? null : (root, query, cb) -> cb.equal(root.get("buyDate"), buyDate);
    }
